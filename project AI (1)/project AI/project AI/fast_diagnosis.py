...
class FastDiagnosisSystem:
    ...
    def extract_entities_and_relationships(self, sentence):
        ...
        has_pattern = r'(\w+(?:\s+\w+)*)\s+has\s+symptoms?\s+(.+)'
        ...
        if match:
            ...
            symptoms = [s.strip().rstrip('.') for s in symptoms_text.split(',')]
...
def main():
    # Create and run the fast diagnosis system
    system = FastDiagnosisSystem()
    
    try:
        # Read and process knowledge
        knowledge = system.read_knowledge_file()
        ...
        print("\n✅ System is working! You can now run the interactive version.")
